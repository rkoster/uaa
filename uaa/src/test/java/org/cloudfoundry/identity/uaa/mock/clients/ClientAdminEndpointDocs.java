...
class ClientAdminEndpointDocs extends AdminClientCreator {
    ...
    private static final FieldDescriptor[] idempotentFields = new FieldDescriptor[]{
            ...
            fieldWithPath("tls-client-auth-sub-template").optional(null).type(STRING)
                    .description("Optional certificate-derived subject template. A nonblank value must contain at least one {claim} placeholder declared in tls-client-auth-claim-mappings; constants are rejected with HTTP 400. Blank means no override. Constant or oversized subject templates that reach token generation fail issuance with HTTP 500 / server_error. Placeholder presence does not guarantee subject uniqueness."),
            fieldWithPath("tls-client-auth-aud-templates").optional(null).type(Arrays.asList(ARRAY, STRING))
                    .description("Optional audience templates, as an array or JSON-encoded array. Supports literal audiences such as sts.amazonaws.com and api://AzureADTokenExchange as well as declared {claim} placeholders."),
            fieldWithPath("tls-client-auth-claim-mappings").optional(null).type(Arrays.asList(ARRAY, STRING))
                    .description("Optional mTLS certificate subject mappings, as an array or JSON-encoded array of {field, pattern, claim} objects. Targets must use custom JWT roots. Reserved roots such as amr, acr, auth_time, client_auth_method, cnf, sub and aud, including dotted targets such as acr.level, are rejected with HTTP 400. Use tls-client-auth-sub-template and tls-client-auth-aud-templates for subject and audience customization. See [client authentication configuration](https://github.com/cloudfoundry/uaa/blob/develop/docs/UAA-Client-Authentication.md#configuration) for the complete reserved list."),
